...
class training():
    def __init__(self, args):
        ...
        self.path_in = args.input
        self.path_out = args.output
        self.quantized = args.quantized

    def get_callbacks(self, path_out, sample_size, batch_size):
        # early stopping callback
        early_stopping = EarlyStopping(monitor='val_loss', patience=20, verbose=1, restore_best_weights=False)

        csv_logger = CSVLogger(f'{self.path_out}loss_history.log')

        # model checkpoint callback
        # this saves our model architecture + parameters into model.h5
        model_checkpoint = ModelCheckpoint(f'{self.path_out}model.h5', monitor='val_loss',
        ...
        lr_scale = 1.
        clr = CyclicLR(base_lr=0.0003*lr_scale, max_lr=0.001*lr_scale, step_size=sample_size/self.batch_size, mode='triangular2')
    ...
    def test(self, Yr_test, predict_test, PUPPI_pt, path_out):
        ...
        MET_rel_error_opaque(predict_test[:,0], PUPPI_pt[:,0], Yr_test[:,0], name=''+self.path_out+'rel_error_opaque.png')
        MET_binned_predict_mean_opaque(predict_test[:,0], PUPPI_pt[:,0], Yr_test[:,0], 20, 0, 500, 0, '.', name=''+self.path_out+'PrVSGen.png')
        
        Phi_abs_error_opaque(PUPPI_pt[:,1], predict_test[:,1], Yr_test[:,1], name=self.path_out+'Phi_abs_err')
        Pt_abs_error_opaque(PUPPI_pt[:,0], predict_test[:,0], Yr_test[:,0],name=self.path_out+'Pt_abs_error')
    ...
    def trainFrom_Root(self):
        ...
        filesList = glob(os.path.join(f'{self.path_in}','*.root'))
    ...
    def trainFrom_h5(self):
        ...
        h5files = []
        for ifile in glob(os.path.join(f'{self.path_in}','*.root')):
